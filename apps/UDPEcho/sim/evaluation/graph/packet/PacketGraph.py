...
class PacketGraph:
    ...
    def execute(self,
        ...
        consist = np.zeros((si.sqr_nodes, si.sqr_nodes))
        ...
        ax.set_ylim(1, si.sqr_nodes*si.sqr_nodes+1)

        if sqr_nodes < 10:
            ax.set_yticks(range(si.sqr_nodes*si.sqr_nodes+1))
        else:
            ax.set_yticks(range(0, si.sqr_nodes*si.sqr_nodes+1, 10))

        ax.set_xlabel('Model time [s]')
        ax.set_ylabel('Node ID')

        plt.title('Packet Scatter')
        text = str(si.sqr_nodes) + "x" + str(si.sqr_nodes) + "\n" + \
        ...
        plt.savefig(filenamebase+"_packet.pdf")
